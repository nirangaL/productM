...
class Location_Dashboard_Con extends MY_Controller{
    ...
    public function getTeamData($teamId){
      ...
      if (!empty($result)) {
        ...
        for($i=0;$i<sizeof($result);$i++){
          ...
          $data['hourlyData'][$i] = $this->getHourlyOut($teamId,$result[$i]->style,$result[$i]->hrs,$result[$i]->timeTemplate,$result[$i]->smv,$result[$i]->noOfwokers,$getTime[0]->currentHour,$getTime[0]->styleStartHour);
        }
        ...
        for($i=$newStyleStartHour;$i<=$totalHour;$i++){
          ...
          if(!empty($result)){
            if(strtotime($currentTime) > strtotime($result[0]->startTime) && strtotime($currentTime) < strtotime($result[0]->endTime)){
              ...
              $result[0]->styleStartHour = new stdClass();
              $result[0]->styleStartHour = $newStyleStartHour;
              return $result;
            }else{
              if($decimalMin !=0){
                if(($i+$decimalMin) == $maxHrs){
                  return  $this->getTeamTimeForDecimalValue($roundDownHour,$maxHrs,$newStyleStartHour);
                }
              }
            }
          }
  ...
  public function getTeamTimeForDecimalValue($roundDownHour,$maxHrs,$styleStartHour){
    $currentTime  = date('H:i:s');
    ...
    if(!empty($result)){
      if(strtotime($currentTime) > strtotime($result[0]->startTime) && strtotime($currentTime) < strtotime($result[0]->endTime)){
        ...
        $result[0]->styleStartHour = new stdClass();
        $result[0]->styleStartHour = $styleStartHour;
        return $data;
      }
    }
  }
    ...
    public function getEfficiency($workingMin,$actualOutQty,$smv,$noOfwokers,$forTotalEffiCal=''){
    ...
    public function roundEff($effi,$roundUpNu){
      ...
      $checkingValue = $roundDownEffi + $roundUpStartNu;
      $decimalValue = (double)$effi - $roundDownEffi;
