...
class Qc_passlog extends MY_Controller{

    function __construct(){
        parent::__construct();
        $this->checkSession();
        $this->load->model('report/Qc_passlog_model');
    }

    public function  index($tableData ='',$team='',$selectedFromDate='',$selectedFromTime='',$selectedToDate='',$selecedToTime=''){

        $data['team'] = $this->Qc_passlog_model->getTeam($this->myConlocation);
        ...
        $this->load->view('report/qc_passlog_view');
    ...
    public function getTableData(){
        ...
        $toTime = $this->input->post('toTime');
         ...
         if($team !='' && $fromDate !=''  && $fromTime !=''  && $toDate !='' && $toTime !=''){
            $result =  $this->Qc_passlog_model->getTableData($team,$fromDate,$fromTime,$toDate,$toTime);
