    ...
    public function getSavedHourlyData($teamID){
        ...
        $sql = "SELECT
            ...
            FROM
                ...
                INNER JOIN `dep_operation` 
                    ON (`worker_in_team`.`operation_id` = `dep_operation`.`id`) WHERE `worker_in_team`.`teamId` ='$teamID' AND `hour`='$hour' AND DATE(createdTime)='$date' ORDER BY id DESC";
        return $this->db->query($sql)->result();
    }
